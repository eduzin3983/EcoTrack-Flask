...
def hill_encrypt(password, key_matrix):
    ...
    # Remove tudo que não for letra e transforma em maiúsculas
    password = re.sub(r'[^A-Z]', '', password.upper())

    # Preenche com 'X' até o tamanho ser múltiplo da ordem da matriz
...
def hill_decrypt(encrypted_password, key_matrix):
    ...
    encrypted_password = re.sub(r'[^A-Z]', '', encrypted_password.upper())
    ...
    determinant_mod_inverse = mod_inverse(determinant, 26)
